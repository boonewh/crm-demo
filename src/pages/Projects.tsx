...
const PROJECT_STATUS_OPTIONS = ['Planning', 'In Progress', 'Completed', 'Cancelled'] as const;
...
export default function Projects() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const projectsData = await (projRes as any).json();
        const clients = await (clientRes as any).json();
        const leads = await (leadRes as any).json();
  ...
  const handleSave = async () => {
    try {
      ...
      const data = await (updated as any).json();
