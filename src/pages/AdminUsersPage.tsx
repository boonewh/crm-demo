...
export default function AdminUsersPage() {
  ...
  const handleCreate = async () => {
    ...
    if (res.ok) {
      const newUser = await (res as any).json();
      setUsers((prev) => [...prev, newUser]);
      setEmail("");
      setPassword("");
      setRole("user");
      setShowForm(false);
      toast.success(`Demo: User "${newUser.email}" created successfully!`);
    } else {
      const { error } = await (res as any).json();
  ...
  const handleToggleActive = async (id: number) => {
    ...
    if (res.ok) {
      const updated = await (res as any).json();
      setUsers((prev) =>
