...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [token, setToken] = useState<string | null>(() => 
    IS_DEMO_MODE ? "demo-jwt-token" : localStorage.getItem("token")
  );
  ...
  const [loading, setLoading] = useState<boolean>(IS_DEMO_MODE ? false : (!!token && !user));
  ...
  const logout = () => {
    // In demo mode, prevent logout and show a message
    if (IS_DEMO_MODE) {
      alert("Demo mode: Logout is disabled. Refresh the page to reset the demo.");
      return;
    }
  ...
  // 🔒 Auto-logout on expiration (disabled in demo mode)
  useEffect(() => {
    if (IS_DEMO_MODE) return; // Skip token expiration checks in demo mode
  ...
  // 🔁 Verify token with retry on app load (disabled in demo mode)
  useEffect(() => {
    if (IS_DEMO_MODE) return; // Skip token verification in demo mode
  ...
  // ⏳ Connecting / Retry / Give Up UI (skipped in demo mode)
